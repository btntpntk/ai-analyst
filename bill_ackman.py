...
def analyze_valuation(financial_line_items: list, market_cap: float) -> dict:
    ...
    Uses a simplified DCF with FCF as a proxy. Falls back to earnings multiple.
    """
    ...
    # Primary Method: FCF-based DCF
    fcf = getattr(latest, 'free_cash_flow', 0) or 0
    if fcf > 0:
        growth_rate = 0.06
        discount_rate = 0.10
        ...
        details = [f"FCF-based IV: ~{intrinsic_value:,.0f}", f"Market Cap: ~{market_cap:,.0f}", f"MoS: {margin_of_safety:.1%}"]
        
        return {"score": score, "details": "; ".join(details), "intrinsic_value": intrinsic_value, "margin_of_safety": margin_of_safety}

    # Fallback Method: Earnings Multiple
    net_income = getattr(latest, 'net_income', 0) or 0
    if net_income > 0:
        reasonable_multiple = 15
        intrinsic_value = net_income * reasonable_multiple
        margin_of_safety = (intrinsic_value - market_cap) / market_cap

        score = 0
        if margin_of_safety > 0.3:
            score += 2  # Less confidence than FCF method
        elif margin_of_safety > 0.1:
            score += 1
            
        details = [f"Fallback Earnings-based IV (15x): ~{intrinsic_value:,.0f}", f"Market Cap: ~{market_cap:,.0f}", f"MoS: {margin_of_safety:.1%}"]

        return {"score": score, "details": "; ".join(details), "intrinsic_value": intrinsic_value, "margin_of_safety": margin_of_safety}

    # If both fail
    return {"score": 0, "details": f"No positive FCF or Net Income for valuation.", "intrinsic_value": None}
...
def generate_ackman_output(
    ...
    template = ChatPromptTemplate.from_messages([
        (
            ...
            For only "reasoning" return in Thai
            """
        ),
        (
            "human",
